...
class Nodelet : public nodelet::Nodelet {
 ...
 public:
  void onInit(void) {
    ...
    quadrotorPtr_->setPos(Eigen::Vector3d(init_x, init_y, init_z)); // position
    quadrotorPtr_->setYpr(Eigen::Vector3d(init_yaw, 0, 0));         // euler
    double rpm = sqrt(config.mass * config.g / 4 / config.kf);
    quadrotorPtr_->setRpm(Eigen::Vector4d(rpm, rpm, rpm, rpm));     // revolutions per minute(无人机电机轴在60s内旋转一个周期的次数)
    
    Eigen::Quaterniond quat = uav_utils::ypr_to_quaternion(Eigen::Vector3d(init_yaw, 0, 0)); // euler to quaternion
