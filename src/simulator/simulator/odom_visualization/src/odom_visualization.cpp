...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (1) {
    ...
    string base_s   = _agent_id == -1 ? string("base")   : string(_agent_nm) + std::to_string(_agent_id) + string("_base");
    string laser_s  = _agent_id == -1 ? string("laser")  : string(_agent_nm) + std::to_string(_agent_id) + string("_laser");
    string vision_s = _agent_id == -1 ? string("vision") : string(_agent_nm) + std::to_string(_agent_id) + string("_vision");
    string height_s = _agent_id == -1 ? string("height") : string(_agent_nm) + std::to_string(_agent_id) + string("_height");
...
int main(int argc, char** argv) {
  ...
  n.param("agent_nm",   _agent_nm, string(""));  
  n.param("agent_id",   _agent_id, -1);  
